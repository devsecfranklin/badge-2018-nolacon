...
/*
 * This function will accept a 8 bit integer to decide 
 * which flash pattern to do.
 * 
 */
void ledCallback(uint8_t myPattern) {
  //uint8_t myPattern = 0;
  if (myPattern = 0) {
...
void screenCallback() {
  ...
  char *myscreen = (char*)malloc(1);
  myscreen = mymenu.getMyMenu();
  display.print(myscreen);
  display.display();
  free(myscreen);
}
...
void setup() {
  ...
  */
  ...
  btn3Thread.onRun(btn1Callback);
  btn3Thread.setInterval(100); 

  // Configure btn2Thread callback and interval
  //btn4Thread.onRun(btn2Callback);
  //btn4Thread.setInterval(200);
  ...
  controll.add(&btn3Thread); // & to pass the pointer to it
//  controll.add(&btn4Thread); 
}
